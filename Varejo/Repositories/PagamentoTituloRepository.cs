...
public class PagamentoTituloRepository : IPagamentoTituloRepository
{
    ...
    public async Task<List<PagamentoTitulo>> GetByTituloIdAsync(int tituloId)
    {
        return await _context.PagamentosTitulo
            ...
            .Where(p => p.TituloFinanceiroId == tituloId).OrderByDescending(p => p.DataPagamento).ToListAsync();
    ...
    public async Task RegistrarPagamentoAsync(int tituloId, decimal valor, DateTime data)
    {
        var titulo = await _context.TitulosFinanceiro
            ...
            .Include(t => t.Pagamentos).FirstOrDefaultAsync(t => t.IdTituloFinanceiro == tituloId);


        if (titulo == null) throw new Exception("Título não encontrado.");


        if (valor <= 0) throw new Exception("Valor inválido.");


        if (valor > titulo.ValorAberto) throw new Exception("Pagamento maior que o saldo.");
        ...
        // 🔥 NÃO adiciona manualmente na lista// titulo.Pagamentos.Add(pagamento); ❌ REMOVE ISSO
